...
- ⚠️ **「禁发横移 VY」目前只是未验证的说法，而且软件侧没有执行**（2026-09-18 复核，同日更正措辞）：软件侧仍按 `ky: 1.2 / max_vy: 0.30`（`robot.yaml:32,38`）下发 `/cmd_vel.linear.y`，路线里也有侧移段（`mission_route.py` S11/S14、`work_sequence.py` 0.15/0.11 m），`cmd_vel_arbiter.py` 不滤 vy——**这部分是代码事实**。但"横移不可用"本身**没有实测支撑**：只有 08-19 的一条当面记录（问答表 C-6，无数字无签字），08-18 架空实测显示 vy 麦轮转向组合**正确**（`RASPBERRY_PI_DEPLOYMENT_LOG_2026-08-18.md:22-24`），固件实现了 vy（`chassis.h:36,73`、`chassis.c:23`），电控侧仍列「vx/vy/wz 实测值」为待测（`给电控组待确认清单.md:101`）。**别把"不可用"当已确认事实**；上车前要做落地横移实测，并决定是否先把 vy 钳 0。
