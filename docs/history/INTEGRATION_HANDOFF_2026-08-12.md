...
> **2026-09-18 盘点注记（历史留痕，不用于执行）**：下文「本文件是**当前唯一集成分支**的正式交接入口」
> 只对 08-12 当天成立。现行主线是 `integration/line-follow-t26`（`integration/robogame-t26` 已是其祖先），
> 当前交接入口见 `docs/README.md`。文件正文里 08-12 的机构/协议结论多处已被后续实现取代
> （文中已就地标注 2026-09-17 更正，例如「唯一集成分支/工作树干净」「机构服务以 `2001` 拒绝」）。
...
~~在契约冻结前，真实机构服务继续以错误码`2001`拒绝请求。~~ **⚠️ 更正（2026-09-17 复核）：此为 2026-08-12 当日状态记录，现已过期**——真实机构服务已实现（`GRAB/RELEASE/HOME/STOP/ARM_SET` 映射 0x20），`2001` 不在任何实现路径中；当前真实拒绝码为 `9003/9001/9006/5/4/6/9`，仅 `LIFT_ABS` 由固件返回 `3010`（本车无升降）。见 `docs/TODO_AND_ISSUES.md` ISSUE-007。
