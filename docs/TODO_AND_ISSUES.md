...
- [ ] `P0` 增加真实模式失败安全测试：没有成功解码 `0x81 RobotStatus` 时，`communication_ok` 不得仅因收到其他合法帧而变为真。
...
- [ ] `P0` 准备单动作验收入口，能够独立调用 `GRAB`、`LIFT`、`RELEASE`、`STOP`，记录请求、耗时、响应和 RobotStatus；在协议未冻结前只连模拟服务。
- [ ] `P0` 为状态过期、服务不可用、服务失败、急停和机构故障准备可复现的测试步骤。
- [ ] `P1` 在 Ubuntu 完整构建并运行机构模拟 smoke，保存终端证据和提交号。
- [ ] `P1` 输出现场第一天执行顺序，禁止一开始运行完整任务。

两天内不做：猜测 `0x02/0x81` 字节载荷、大规模改成 ROS2 Action、增加未经机械确认的新动作、调真实三层高度。
...
### ISSUE-007：真实串口机构与状态载荷尚未定义

- 优先级：`P0`；
- 现状：`0x02` 机构命令、`0x81` RobotStatus、`0x82` 里程计和 `0x83` IMU 只有消息类型，没有逐字节字段表；
- 当前保护：`robot_bridge` 在非模拟模式主动拒绝机构服务，错误码为2001；
- 风险：擅自猜测字段会导致方向、单位、超时、重复命令和错误恢复不一致；
- 所需决定：字段偏移、类型、单位、范围、命令序号、应答匹配、幂等、取消和版本兼容。
...
- 远程可完成：增加失败安全判据与测试，真实状态适配器未完成前保持不健康。
...
- 优先级：`P0`；
- 现状：`action_timeout_s=12.0`，单服务请求常用3秒，稳定观察需要3秒；
- 风险：真机正常但较慢时，整体超时可能先于稳定观察完成；
- 远程可完成：建立预算计算和配置校验；
- 现场数据：升降、释放、撤退、状态回传的正常值、P95值和最大安全值。
