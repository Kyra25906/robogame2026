...
- 巡线真实 `0x14` 遥测**协议已实现但字节布局尚未与电控书面冻结**：固件侧 `RPI_SendLineTelemetry()`（`rpi_protocol.c:1552`，在 `RPI_Update` 内周期上送）与上位机侧解码（`serial_protocol.py:29`、`robot_bridge/node.py:1096` → `/line_sensor`）均已就位。当前面板默认数据源仍是 mock，**换真适配器需真车验收**；数据是模拟值时不能当真车证据。
