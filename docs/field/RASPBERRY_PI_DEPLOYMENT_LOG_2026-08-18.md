...
2. **里程计 10 倍偏差**：命令 1m 实际只走 ~10cm——**怀疑固件轮径/编码器换算参数错一个数量级**（`chassis.h` 的 `WHEEL_RADIUS_M 0.06` 等需实测），需电控确认轮径/PPR/减速比。
   ...
   > - `ENCODER_COUNTS_PER_REV`（`encoder.h:14`，现 `1404.0f`）——**该值是按「13 PPR × 4 倍频 × 27 减速比」算的，并注明「2026-09-13 实测手转 10 圈约 1395 计数/圈」**。⚠️ **这个实测是换电机之前做的，换电机后 27:1 这个前提可能已经不成立**（新电机若为 36:1，则该常量应为 `13×4×36 = 1872`）。**必须在当前硬件上手转 10 圈重新量一次**，这是判断里程计标度最直接的一步。
