...
2. **里程计 10 倍偏差**：命令 1m 实际只走 ~10cm——**怀疑固件轮径/编码器换算参数错一个数量级**（`chassis.h` 的 `WHEEL_RADIUS_M 0.06` 等需实测），需电控确认轮径/PPR/减速比。
   > **2026-09-17 用固件源码复核（重要，纠正上面的怀疑方向）**：**电控源码就在工作树里**（`Four_Motor_PID_Test_1/Four_Motor_PID_Test/Core/`，被 `.gitignore` 忽略所以不在 Git 里，但不是没有）。逐条核对后：
   > - **「参数错一个数量级」这个方向几乎可以排除。** 固件里 `CHASSIS_WHEEL_RADIUS_M` **同时**用在两个方向——`Chassis_SetVelocityPhysical()`（`chassis.c:303-318`，机体速度→轮角速度）和 `Chassis_GetBodyVelocity()`（`:403-427`，轮角速度→机体速度）。两者用的是**同一个常量**，所以轮径误差在命令与反馈之间**互相抵消**：报出来的速度会与命令一致，闭环照样"自认为走到了"。**它造成的是绝对距离缩放（1.067 倍），不是 10 倍。**
   > - 现值 `CHASSIS_WHEEL_RADIUS_M 0.06f`（合径 120 mm）vs 08-19 冻结 **128 mm**（半径 0.064）→ 只差 **1.067 倍**；半轴距 0.2425（轴距 485）vs 冻结 475、半轮距 0.2375（轮距 475）vs 冻结 465（`chassis.h:54-55`）。
   > - `ENCODER_COUNTS_PER_REV 1404.0f`（`encoder.h:14`）= 13 PPR × 4 倍频 × **27** 减速比，**且标注「2026-09-13 实测手转 10 圈约 1395 计数/圈，与此一致」**——即该常量已被实机验证过，**说明 27:1 是对的，"1:36" 反而可疑**。所以「减速比错」也不支持 10 倍。
   > - 结论：**10 倍偏差更可能是别处**（速度标度/时间步/某处 ×10 或 ÷10），而不是轮径或减速比。查的时候**先按 1.07 与 1.33 的量级排除参数，再去找数量级级别的错**。`tools/odom_calibration.py` 的分层判据（先看里程计自洽性，再看尺量标度）正好适用于此。
