...
- [ ] E3 落地标定：**直行 1m / 原地转 360°** ×10（等固件 odom 修好 + 轮径确认）
  ...
  - ⚠️ 另注意：本项与里程计 **~10× 尺度误差**纠缠（`RASPBERRY_PI_DEPLOYMENT_LOG_2026-08-18.md:87`，命令 1 m 实际约 10 cm），**未修好前「直行 1m」测的是标度而不是标定**；且固件换算常数与 08-19 冻结值不一致（轮半径 120 vs 128 mm、编码器 27:1 vs 1:36，`chassis.h:53`/`encoder.h:11-14`）。详见 `INTEGRATION_CHECKLIST.md` 的 BLOCKED 条与 `tools/odom_calibration.py`。
