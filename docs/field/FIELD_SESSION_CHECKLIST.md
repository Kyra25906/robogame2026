...
- [ ] E3 落地标定：**直行 1m / 原地转 360°** ×10（等固件 odom 修好 + 轮径确认）
  - ⚠️ **原写「横移 1m」，已于 2026-09-17 删除**：2026-08-19 机械当面确认真车**不能横移**（左右平移偏差过大，C-6；`真车对接设计稿_2026-08-19.md:25,113` → **VY 恒 0**）。照着测会得到一堆无法解释的偏差。
  - ⚠️ **换电机了，所以里程计要重新判定（2026-09-17 用户补充）**：2026-08-18 旧电机时期有过 ~10× 尺度误差（命令 1 m 实际约 10 cm），但那是**旧硬件记录**。换电机后 `ENCODER_COUNTS_PER_REV`（现 1404，09-13 旧硬件手转实测）与轮径都可能失效，**不要预设它坏、也不要预设它好**。
    - 先做**不用尺子**的一步：网页面板看「里程计等效速度 ÷ 命令速度」是否 ≈1（自洽）；
    - 再做需要尺子的一步：0.50 m / 0.05 m·s⁻¹ 定距跑三次，尺量实际位移定标度；
    - 源码里 `CHASSIS_WHEEL_RADIUS_M`（现 0.06f）在命令与反馈**两侧同用**、互相抵消，所以**它不是标度错的来源**；标度错更可能来自 `ENCODER_COUNTS_PER_REV`（减速比单向进入换算）。详见 `RASPBERRY_PI_DEPLOYMENT_LOG_2026-08-18.md` 第 7.3 节的复核段与 `tools/odom_calibration.py`。
