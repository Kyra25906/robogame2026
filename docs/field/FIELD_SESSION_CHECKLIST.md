...
- [ ] E3 落地标定：**直行 1m / 原地转 360°** ×10（等固件 odom 修好 + 轮径确认）
  - ⚠️ **原写「横移 1m」，已于 2026-09-17 删除**：2026-08-19 机械当面确认真车**不能横移**（左右平移偏差过大，C-6；`真车对接设计稿_2026-08-19.md:25,113` → **VY 恒 0**）。照着测会得到一堆无法解释的偏差。
    - ✏️ **2026-09-18 晚更正（证据核对）**：上面这条"机械当面确认不能横移"**没有实测支撑**——C-6 只是当面记录、无数字无签字；08-18 架空实测反而显示 vy 的麦轮转向组合**正确**（`RASPBERRY_PI_DEPLOYMENT_LOG_2026-08-18.md:22-24`），固件也实现了 vy（`chassis.h:36,73`、`chassis.c:23`），电控侧仍列"vx/vy/wz 实测值"为待测（`给电控组待确认清单.md:101`）。所以**删掉「横移 1m」这一步本身也值得重新考虑**：它恰恰是判定"横移到底能不能用"的唯一手段。当前建议：先做完直行标定（上面两条），再单独用低速短路程试横移 0.2～0.3 m ×5 量偏差，**只看现象、不下结论**。
