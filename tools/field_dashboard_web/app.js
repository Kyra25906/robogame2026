...
async function refresh(){try{const s=await (await fetch('/api/snapshot',{cache:'no-store'})).json();render(s);renderDistance(s);renderGrasp(s);renderCalibration(s);renderRoute(s);renderOdom(s);renderRuntimeGuard(s)}catch(e){$('#lineDiagnosis').textContent='网页状态连接失败：以下为旧值，不能判断当前车辆状态';$('#lineBars').style.opacity='.35';toast('状态连接失败：'+e.message,true)}}
